...
void setup() {
  ...
  servoTop.write(topMotorNoHittingPosition); //start from No Hitting for top motor
  servoTop.attach(servoTopPin);
  delay(500);

  servoBottom.write(bottomMotorUpPosition);
  servoBottom.attach(servoBottomPin);
  delay(500);

  servoMiddle.write(middleMotorUpPosition);
  servoMiddle.attach(servoMiddlePin);
  delay(500);

  delay(3000);

  //Make water in tubes go all the way to the faucet mouth
